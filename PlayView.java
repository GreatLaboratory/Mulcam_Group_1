...
import game.Cal; // 채점 및 변환 프로그램과 연동
import game.InputForm;
...
//	
	...
	public JLabel la_life, la_level, la_score, la_best, la_time, la_notice;
//	
...
//	static String SCORE ;
	...
	public PlayView(InputForm form) {
		dao = new WordsDAO();
		LowerPanel = new JPanel();
		UpperPanel = new JPanel();
		PlayPanel = new JPanel();

		c = new Cal(this,form); // 채점 연산자
		...
		Timer timer = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (PlayView.sec == 10) {
					t_array[1].start();
					d_array[1].setVisible(true);
				}
				else if (PlayView.sec == 15) {
					t_array[2].start();
					d_array[2].setVisible(true);
				}
				else if (PlayView.sec == 20) {
					//new InputForm(); // 임시로 만들어놓은거임
					t_array[3].start();
					d_array[3].setVisible(true);
				}
				la_time.setText("" + sec);
